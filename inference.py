...
def evaluate(label_file,pb_path, n=None, threshold=None,shuffle=True, save_dir=None,out_label=None):
    ...
    for i, line in enumerate(lines):
        ...
        if n and i == n:
            break
    t2=time.time()
    fps = float(len(lines))/(t2-t1)
    print "fps is ",fps
    y_prob = np.array(y_prob)
    report = classification_report(y_true, y_pred, digits=4, target_names=['n','s','p']) 
...
if __name__ == '__main__':
    ...
    parser.add_argument( '-l', '--test_label_file', default="label/test3.txt",dest= 'test_label_file', action= 'store', help= 'the label file contains the test images. only valid when phase is test') 
    ...
    parser.add_argument( '-output_file', default= 'result/three_fast2.txt',dest="output_file") 
    parser.add_argument('-aw', '--acc_weight_path', default="ckpt/acc_three.pb",dest= 'a_weight_path', action= 'store', help= 'the pretrained weight_path') 
    parser.add_argument('-fw', '--fast_weight_path', default="ckpt/fast_three.hdf5",dest= 'f_weight_path', action= 'store', help= 'the pretrained weight_path') 
    ...
    if args.mode==1:
        ...
        model = Global_Net(input_dim=(299,299,3),output_dim=3, verbose=0, weight_path=args.f_weight_path) 
        model.evaluate(args.test_label_file, limit_number=0, threshold=0., target_names=['normal','sexy','porn'], num_class=3, balance=False,out_label= args.output_file) 
    else:
