...
class MyImageDataGenerator(ImageDataGenerator):
    ...
    def fit_generator(self, generator, steps, save_file='./data/image_channel_mean_std.npy'):
    ...
    def num_samples_per_epoch(self, label_file_path, limit_number=None, balance=True):
        sample_dict = {}
        ...
        if balance:
            ...
            print ( "after balanced, total sample is {}".format(num_class*min_n) ) 
        if limit_number:
    ...
    def flow_from_label_file(self, label_file_path, balance=True, nb_output=1, limit_number=None, target_size=(299,299), batch_size = 32, shuffle=True, seed = None, phase='train',is_ergodic_files=False):
...
class LabelFileIterator(Iterator):
    ...
    def __init__(self, label_file_path,image_data_generator, balance=True, nb_output=1, limit_number=None, target_size=(299,299), batch_size = 32, shuffle=True, seed = None, phase='train',is_ergodic_files=False):
        ...
        num_per_epoch, sample_dict = self.image_data_generator.num_samples_per_epoch(label_file_path, limit_number=limit_number, balance=balance)
	...
	self.phase= phase
        ...
        super(LabelFileIterator, self).__init__(num_per_epoch, batch_size, shuffle, seed)
        self.broken_recorder = 'label/brokenImages.log'
    ...
    def gen_data_path_list(self):
        ...
        for i in self.sample_dict:
            if self.balance:
                sample = np.random.choice( self.sample_dict[i], self.num_per_class, replace=False) 
            else:
                sample = self.sample_dict[i]
            sample_y = [i]*len(sample) 
            x_path_list.extend(sample) 
            y_list.extend(sample_y) 
        x_path_list, y_list = np.array(x_path_list), np.array(y_list, dtype= 'int32')
        if self.shuffle:
    ...
    def gen_ergodic_data_path_list(self):
        ...
        for i in self.sample_dict:
	    if i == 0:
		sample = self.selectOpenImages( self.sample_dict[i], self.num_per_class, self.balance,self.openImageIndex)
	    else:
		if self.balance:
		    sample = np.random.choice( self.sample_dict[i], self.num_per_class, replace=False) 
                else:
	...
	sample1_selected = [sample1[i] for i in self.openImageIndex[(epoch%13)*250000:((epoch+1)%13)*250000]]
	if balance:
            print(len(sample1),len(sample1_selected),len(sample2),self.num_per_class)
	    return np.random.choice(sample1_selected+sample2,self.num_per_class,replace=False)
	else:
    ...
    def next(self):
        ...
        for i,label in enumerate( self.y_list[ index_array] ):
            batch_y[i, label] = 1.
        if self.nb_output ==1:
            return batch_x, batch_y,fns
        else:
            return batch_x, [batch_y]*self.nb_output,fns
